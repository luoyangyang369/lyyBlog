...
- @ConfigurationProperties: 解析配置文件读取并与 bean 绑定
- @RequestMapping： 请求路由
...
- 字符串常量池： 堆内唯一
- 静态常量池： 编译阶段的 class 信息， 比如编译时期的字面量和符号引用， 这部分内容将在类加载后放到 运行时常量池
- 运行时常量池： 一个类一个， 类加载完成后， 会将符号引用解析为直接引用等， 然后把 静态常量池信息 放到 运行时常量池
